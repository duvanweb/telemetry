...
import { findByPlate } from '@/api/vehicle';
...
export default function SearchScreen() {
  ...
  const errorMessage = (() => {
    ...
    if (error instanceof ApiError && error.status === 404) return 'Vehicle not found';
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      <View className="flex-1 px-6 py-8">
        <Text className="text-2xl font-bold text-slate-900">Search by plate</Text>
